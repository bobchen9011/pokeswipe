...
// 取得前三張圖片
    ...
    // 反轉順序是為了讓 HTML 先畫底層卡片，但我們要重新計算 depth
    topCards.slice().reverse().forEach((img, revIdx) => {
      
      // 核心修正：確保原陣列第一張（新圖）的 depth 永遠是 0
      const depth = topCards.length - 1 - revIdx;
      
      const card = buildCard(img, depth);
      cardStack.appendChild(card);
      
      // 只有當 depth 是 0 時，才是視覺上最前面的那張卡片
      if (depth === 0) {
